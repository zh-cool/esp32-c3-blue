...
static int desc_access(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg)
{
	const char *name = (const char *)arg;
	return os_mbuf_append(ctxt->om, name, strlen(name)) ? BLE_ATT_ERR_INSUFFICIENT_RES : 0;
}
...
static void adv_start(void)
{
	...
	if (rc != 0) {
		ESP_LOGE(TAG, "adv_set_fields fail (rc=%d)", rc);
		return;
	}
